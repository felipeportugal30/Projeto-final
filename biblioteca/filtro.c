...
unsigned char *matrizFiltro(unsigned char *imagem, int TAM, int LER){
  int iMagem[TAM][TAM];
  for(int i=0; i<TAM; i++){
    for(int j=0; j<TAM; j++){
  ...
  unsigned char *paraFiltrar = (unsigned char *)calloc((TAM+LER/2*2)*(TAM+LER/2*2), sizeof(unsigned char));
  if(!(paraFiltrar)){
